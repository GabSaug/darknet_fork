...
# 4 "src/option_list.c" 2
# 1 "src/option_list.h" 1
        ...
        switch(line[0]){
            ...
            default:
                if(!read_option(line, options)){
                    fprintf(
# 25 "src/option_list.c" 3 4
                           stderr
# 25 "src/option_list.c"
...
metadata get_metadata(char *file)
{
    ...
    if(!name_list) {
        fprintf(
# 43 "src/option_list.c" 3 4
               stderr
# 43 "src/option_list.c"
...
void option_unused(list *l)
{
    ...
    while(n){
        ...
        if(!p->used){
            fprintf(
# 85 "src/option_list.c" 3 4
                   stderr
# 85 "src/option_list.c"
                         , "Unused field: '%s = %s'\n", p->key, p->val);
        }
        n = n->next;
    }
...
char *option_find_str(list *l, char *key, char *def)
{
    ...
    if(v) return v;
    if(def) fprintf(
# 108 "src/option_list.c" 3 4
                   stderr
# 108 "src/option_list.c"
                         , "%s: Using default '%s'\n", key, def);
    return def;
}
...
int option_find_int(list *l, char *key, int def)
{
    ...
    if(v) return atoi(v);
    fprintf(
# 116 "src/option_list.c" 3 4
           stderr
# 116 "src/option_list.c"
                 , "%s: Using default '%d'\n", key, def);
    return def;
}
...
float option_find_float(list *l, char *key, float def)
{
    ...
    if(v) return atof(v);
    fprintf(
# 138 "src/option_list.c" 3 4
           stderr
# 138 "src/option_list.c"
                 , "%s: Using default '%lf'\n", key, def);
    return def;
}
