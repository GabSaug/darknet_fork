# 0 "examples/captcha.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "examples/captcha.c"
...
void fix_data_captcha(data d, int mask)
{
    ...
    for(i = 0; i < d.y.rows; ++i){
        for(j = 0; j < d.y.cols; j += 2){
            if (mask){
                if(!labels.vals[i][j]){
                    labels.vals[i][j] = -1234;
                    labels.vals[i][j+1] = -1234;
...
# 109 "examples/captcha.c" 3 4
                                     stdin
# 109 "examples/captcha.c"
                                          );
            if(!input) return;
            strtok(input, "\n");
        }
        ...
        printf("\n");
        fflush(
# 124 "examples/captcha.c" 3 4
              stdout
# 124 "examples/captcha.c"
                    );
        free_image(im);
        if (filename) break;
    }
...
void valid_captcha(char *cfgfile, char *weightfile, char *filename)
{
    ...
    for(i = 0; i < N; ++i){
        if (i%100 == 0) fprintf(
# 143 "examples/captcha.c" 3 4
                               stderr
# 143 "examples/captcha.c"
                                     , "%d\n", i);
        ...
        if (truth == -1){
            fprintf(
# 153 "examples/captcha.c" 3 4
                   stderr
# 153 "examples/captcha.c"
                         , "bad: %s\n", paths[i]);
            return;
        }
        printf("%d, ", truth);
        ...
        printf("\n");
        fflush(
# 162 "examples/captcha.c" 3 4
              stdout
# 162 "examples/captcha.c"
                    );
        free_image(im);
        if (filename) break;
    }
}
# 335 "examples/captcha.c"
void run_captcha(int argc, char **argv)
{
    if(argc < 4){
        fprintf(
# 338 "examples/captcha.c" 3 4
               stderr
# 338 "examples/captcha.c"
