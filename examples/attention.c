# 0 "examples/attention.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "examples/attention.c"
...
# 5 "examples/attention.c" 2


# 6 "examples/attention.c"
...
#pragma omp parallel for
            for(j = 0; j < ngpus; ++j){
                int index = i*ngpus + j;
                extend_data_truth(tiles+index, divs*divs, -1234);
...
void predict_attention(char *datacfg, char *cfgfile, char *weightfile, char *filename, int top)
{
    ...
    while(1){
        if(filename){
            ...
            fflush(
# 405 "examples/attention.c" 3 4
                  stdout
# 405 "examples/attention.c"
                        );
            input = fgets(input, 256, 
# 406 "examples/attention.c" 3 4
                                     stdin
# 406 "examples/attention.c"
                                          );
            if(!input) return;
            strtok(input, "\n");
        }
        ...
        fprintf(
# 420 "examples/attention.c" 3 4
               stderr
# 420 "examples/attention.c"
...
void run_attention(int argc, char **argv)
{
    if(argc < 4){
        fprintf(
# 437 "examples/attention.c" 3 4
               stderr
# 437 "examples/attention.c"
